namespace ZData01.Actions
{
	...
	public class Log(string? message, LogType ltype, BlockType btype, ExType etype, DateTime logMoment, [CallerMemberName] string name = "", [CallerFilePath] string path = "", [CallerLineNumber] int line = default) : IDisposable
	{
		...
		public static void Event(string message, ExType etype = ExType.Base, [CallerMemberName] string name = "", [CallerFilePath] string path = "", [CallerLineNumber] int line = default) => new Log(message, LogType.Event, Def.BType, etype, DateTime.Now, name, path, line).Write();
		...
		public static void Error(Exception ex, StackFrame sf, [CallerFilePath] string path = "")
		{
			try
			{
				...
				{
					if (primconst == null)
						new Log(ex.Message, LogType.Error, Get.BlockType(sf), Def.ExType, DateTime.Now, sf.GetMethod()!.Name, sf.GetFileName() ?? path, sf.GetFileLineNumber()).Write();
					else
						new Log(ex.Message, LogType.Error, BlockType.Primary_Constructor, Def.ExType, DateTime.Now, sf.GetMethod()!.Name, sf.GetFileName() ?? path, sf.GetFileLineNumber()).Write();
				}
			}
			catch (Exception e)
			{
