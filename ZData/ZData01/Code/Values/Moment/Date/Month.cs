namespace ZData01.Values
{
	...
	public class Month : BaseNumber<int>, IMinMaxValue<Month>
	{
		public new static Month MinValue => new(DateOnly.MinValue.Month);
		public new static Month MaxValue => new(DateOnly.MaxValue.Month);
		...
		/// <inheritdoc cref="Month(uint)"/>
		public Month(Month value) : this(value.Value) => Log.Event(new StackFrame(true));

		/// <inheritdoc cref="Month(uint)"/>
		public Month(Date value) : this(value.Value.Month) => Log.Event(new StackFrame(true));

		/// <inheritdoc cref="Month(uint)"/>
		public Month(Moment value) : this(value.Value.Month) => Log.Event(new StackFrame(true));
