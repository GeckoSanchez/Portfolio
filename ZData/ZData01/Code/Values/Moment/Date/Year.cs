namespace ZData01.Values
{
	...
	public class Year : BaseNumber<int>, IMinMaxValue<Year>
	{
		public new static Year MinValue => new(DateOnly.MinValue.Year);
		public new static Year MaxValue => new(DateOnly.MaxValue.Year);
		...
		/// <inheritdoc cref="Year(uint)"/>
		public Year(Year value) : this(value.Value) => Log.Event(new StackFrame(true));

		/// <inheritdoc cref="Year(uint)"/>
		public Year(Date value) : this(value.Value.Year) => Log.Event(new StackFrame(true));

		/// <inheritdoc cref="Year(uint)"/>
		public Year(Moment value) : this(value.Value.Year) => Log.Event(new StackFrame(true));
