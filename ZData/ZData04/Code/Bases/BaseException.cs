namespace ZData04.Bases
{
	...
	public class BaseException : Exception
	{
		...
		public BaseException(Exception? ex, StackFrame sf, ExceptKind exceptiontype = ExceptKind.Base, [CallerMemberName] string name = "", [CallerFilePath] string path = "", [CallerLineNumber] int line = 0) : this(ex, Get.BlockKindOrDefault(sf, Def.BlockKind), exceptiontype, sf.GetMethod()!.Name ?? name, sf.GetFileName() ?? path, line) => Log.Event(new StackFrame(true));

		/// <inheritdoc cref="BaseException(Exception?, StackFrame, ExceptKind, string, string, int)"/>
		/// <param name="message">The given message</param>
		public BaseException(string? message, StackFrame sf, ExceptKind exceptiontype = ExceptKind.Base, [CallerMemberName] string name = "", [CallerFilePath] string path = "", [CallerLineNumber] int line = 0) : this(new Exception(message), Get.BlockKindOrDefault(sf, Def.BlockKind), exceptiontype, sf.GetMethod()!.Name ?? name, sf.GetFileName() ?? path, sf.GetFileLineNumber() != 0 ? sf.GetFileLineNumber() : line) => Log.Event(new StackFrame(true));
